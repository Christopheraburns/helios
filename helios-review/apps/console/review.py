...
runs/<run>/review.json, and publishes the accepted subset as an Ossie model to models/published/.
"""
...
PUBLISH_DIR = os.path.join(runstore.ROOT, "models", "published")
...
def review_page(request: Request, run_id: str, published: str = "", errors: str = ""):
    ...
    published_path = os.path.join(PUBLISH_DIR, f"{model_name}.ossie.yaml")
    last_published = None
    if os.path.exists(published_path):
        last_published = datetime.fromtimestamp(os.path.getmtime(published_path), tz=timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    return _render(request, "review.html", run_id=run_id, p=prop, review=review,
...
def publish(request: Request, run_id: str, commit: str = Form("")):
    ...
    doc, problems = build(reviewed, model_name,
                          description=f"Semantic model for {model_name}, generated by helios run {run_id}", run_id=run_id)
    ...
    yaml_path = os.path.join(PUBLISH_DIR, f"{model_name}.ossie.yaml")
    with open(yaml_path, "w") as f:
        f.write(dump_yaml(doc))
    with open(os.path.join(PUBLISH_DIR, f"{model_name}.ossie.json"), "w") as f:
        json.dump(doc, f, indent=2)
    with open(os.path.join(PUBLISH_DIR, f"{model_name}.publish.json"), "w") as f:
        json.dump({"run_id": run_id, "published_at": datetime.now(timezone.utc).isoformat(),
                   "datasets": len(doc["datasets"]), "relationships": len(doc["relationships"]),
                   "metrics": len(doc["metrics"]),
                   "review": reviewed.get("review")}, f, indent=2)

    msg = f"published {model_name}: {len(doc['datasets'])} datasets, {len(doc['relationships'])} relationships, {len(doc['metrics'])} metrics"
    if commit:
        try:
            subprocess.run(["git", "add", "models/published", f"runs/{run_id}/review.json"], cwd=runstore.ROOT, check=True, capture_output=True)
            subprocess.run(["git", "commit", "-m", f"publish {model_name} from run {run_id}"], cwd=runstore.ROOT, check=True, capture_output=True)
            msg += "; committed"
